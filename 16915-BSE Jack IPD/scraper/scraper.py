...
#                 # Define mappings manually to ensure correct selection
#                 segment_instrument_mapping = {
#                     "Index Derivative": ["Index Futures", "Index Options"],
#                     "Equity Derivative": ["Equity Futures", "Equity Options"]
#                 }

                

#                 # Iterate through each segment and instrument
#                 for segment_value, segment_name in segment_options.items():
#                     if segment_name in segment_instrument_mapping:  # Ensure it's one of our desired segments
#                         for instrument in segment_instrument_mapping[segment_name]:
#                             instrument_value = next((k for k, v in instrument_options.items() if v == instrument), None)
#                             if instrument_value:
#                                 # Now we have the correct segment and instrument combination
#                                 print(f"📡 Fetching data for Segment: {segment_name}, Instrument: {instrument} From Date: {from_date} to {to_date}...")
                                
#                                 # Prepare form payload
#                                 data = {
#                                     'ctl00$ContentPlaceHolder1$ddlsegment': segment_value,
#                                     'ctl00$ContentPlaceHolder1$ddlIntrument': instrument_value,
#                                     'ctl00$ContentPlaceHolder1$ddlUnderLine': '0',
#                                     'ctl00$ContentPlaceHolder1$txtDate': from_date,
#                                     'ctl00$ContentPlaceHolder1$txtTodate': to_date,
#                                     'ctl00$ContentPlaceHolder1$btnGo': 'Go',
...
#                                             if len(cols) >= 5 and "Trade DateNum" not in cols[0].text:
#                                                 trade_date = cols[0].text.strip()
#                                                 num_trades = cols[1].text.strip().replace(',', '')  
#                                                 volume = cols[2].text.strip().replace(',', '')  
#                                                 notional_turnover = cols[3].text.strip().replace(',', '')  
#                                                 premium_turnover = cols[4].text.strip().replace(',', '')  
...
#                                                     premium_turnover
#                                                 ])
#                                                 print(f"✅ Data saved for {segment_name} - {instrument} From Date: {from_date} to {to_date}...")

#                                     else:
#                                         print(f"⚠️ No data found for {segment_name} - {instrument} From Date: {from_date} to {to_date}...")

#                         else:
#                             print(f"❌ Failed request for {segment_name} - {instrument} From Date: {from_date} to {to_date}...")
